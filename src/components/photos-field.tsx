...
export function PhotosField({
  ...
  const add = async (source: 'camera' | 'library') => {
    ...
    try {
      ...
      Alert.alert('Could not add the photo', err instanceof Error ? err.message : 'Please try again.');
    } finally {
      setBusy(false);
    }
  };
    ...
    <View style={styles.wrap}>
      {value.length > 0 ? (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.row}>
          {value.map((url, index) => (
            <Pressable key={url} onPress={() => remove(url)} style={styles.thumbWrap}>
              <Image source={url} style={styles.thumb} contentFit="cover" transition={150} />
              {index === 0 ? (
                <View style={[styles.coverTag, { backgroundColor: theme.primary }]}>
        ...
        <View style={[styles.placeholder, { borderColor: theme.border, backgroundColor: theme.surface }]}>
          <SymbolView name="photo.on.rectangle" size={30} tintColor={theme.textSecondary} />
