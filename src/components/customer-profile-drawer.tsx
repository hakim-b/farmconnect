...
function splitName(profile: { first_name: string | null; last_name: string | null; display_name: string | null } | null) {
  ...
  return (
    <>
      <Pressable
        ...
        style={({ pressed }) => [styles.trigger, { borderColor: theme.border, opacity: pressed ? 0.7 : 1 }]}>
        {profile?.avatar_url ? (
          <Image source={profile.avatar_url} style={styles.triggerImage} contentFit="cover" />
        ) : (
          <View style={[styles.triggerFallback, { backgroundColor: theme.backgroundSelected }]}>
...
function ProfileDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const dirty = first.trim() !== saved.first.trim() || last.trim() !== saved.last.trim();
  ...
  return (
    <Modal visible transparent animationType="none" onRequestClose={onClose}>
      <View style={styles.root}>
        ...
        <Animated.View style={[styles.panelWrap, { width: panelW, transform: [{ translateX }] }]}>
          <ThemedView type="surface" style={styles.panel}>
            ...
            <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
              <View style={styles.avatarBlock}>
                {profile?.avatar_url ? (
                  <Image source={profile.avatar_url} style={styles.avatar} contentFit="cover" transition={150} />
                ) : (
                  <View style={[styles.avatar, styles.avatarFallback, { backgroundColor: theme.backgroundSelected }]}>
                    <SymbolView name="person.fill" size={36} tintColor={theme.textSecondary} />
                  </View>
                )}
                {busy === 'photo' ? (
              ...
              <View style={styles.photoButtons}>
                <Pressable
                  ...
                  disabled={busy != null}
                  style={[styles.photoBtn, { borderColor: theme.border, opacity: busy ? 0.5 : 1 }]}>
                  <SymbolView name="camera.fill" size={16} tintColor={theme.primary} />
                ...
                <Pressable
                  ...
                  disabled={busy != null}
                  style={[styles.photoBtn, { borderColor: theme.border, opacity: busy ? 0.5 : 1 }]}>
                  <SymbolView name="photo.on.rectangle" size={16} tintColor={theme.primary} />
                  <ThemedText type="small">{profile?.avatar_url ? 'Change photo' : 'Add photo'}</ThemedText>
                </Pressable>
              </View>
...
const styles = StyleSheet.create({
  ...
  panel: { flex: 1, borderTopLeftRadius: Radius.lg, borderBottomLeftRadius: Radius.lg },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
